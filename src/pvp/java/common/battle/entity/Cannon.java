...
public class Cannon extends AtkModelAb {
    ...
    public void update() {
        ...
        if (anim == null && atka == null && exta == null) {
            if (id == 3 || id == 4) {
                pos = b.ownBase().pos;
                for (Entity e : b.le)
                    if (e.dire == getDire() && (e.pos - pos) * getDire() > 0 && (e.touchable() & (TCH_N | TCH_KB)) != 0)
                        pos = e.pos;
                pos += getDire() * NYRAN[id] / 2.0f;
            } else if (id == 2 || id == 6) {
                pos = b.isPvp() ? b.getBase(getDire()).pos : Math.max(800f, b.ebase.pos);
                for (Entity e : b.le)
                    if (e.dire == -getDire() && (e.pos - pos) * getDire() < 0 && (e.touchable() & (TCH_N | TCH_KB | TCH_CORPSE | TCH_SOUL)) != 0)
                        pos = e.pos;
            }
        }
        ...
        if (preTime == -1 && id == 2) {
            ...
            wall.added(getDire(), (int) (pos - getDire() * 100)); // guessed distance from enemy compared from BC // update: checked with GG, correct
    ...
    private float getBreakerSpawnPoint(float pos, float range) {
        return pos - getDire() * (float) Math.ceil(range * 4 / 5) / 4f;
    }
