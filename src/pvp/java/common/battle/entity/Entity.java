...
public abstract class Entity extends AbEntity {
    public long pvpEntityId;
    @Override public void added(int direction, float position) {
        super.added(direction, position);
        if (basis.isPvp() && pvpEntityId == 0) pvpEntityId = basis.allocateEntityId();
    }
	...
	private static class ZombX extends BattleObj {
		...
		private final Set<Entity> list = new java.util.LinkedHashSet<>();
	 ...
	 */
	public Set<AttackAb> lastHitBy = new java.util.LinkedHashSet<>();

	/**
	 * Attacks it got hit by on the frame of KB into death.
	 */
	public Set<AttackAb> lastKilledBy = new java.util.LinkedHashSet<>();
	...
	public boolean damaged(AttackAb atk) {
		...
		if (basis.isPvp() ? this instanceof EEnemy : dire == 1) {
		...
		atk.notifyEntity(e -> {
			...
			if(e instanceof EUnit && ((EUnit) e).index != null) {
				...
				(basis.isPvp() ? e.basis : basis).totalDamageGiven[index[0]][index[1]] += Math.min(d, health);
			}
		});
	...
	public boolean processProcs(AttackAb atk) {
		...
		if (atkProc.POIATK.mult > 0) {
			...
			if (rst == 100) {
				...
				if ((this.dire == -1 || basis.isPvp()) && basis.canon.deco == DECO_BASE_BARRIER)
		...
		if (atkProc.STOP.time != 0 || atkProc.STOP.prob > 0) {
			...
			if((this.dire == -1 || basis.isPvp()) && basis.canon.deco == DECO_BASE_STOP) {
		...
		if (atkProc.SLOW.time != 0 || atkProc.SLOW.prob > 0) {
			...
			if((this.dire == -1 || basis.isPvp()) && basis.canon.deco == DECO_BASE_SLOW) {
		...
		if (atkProc.WEAK.time > 0) {
			...
			val = (int) (val * rst);
			if((this.dire == -1 || basis.isPvp()) && basis.canon.deco == DECO_BASE_GROUND) {
		...
		if (atkProc.CURSE.time != 0 || atkProc.CURSE.prob > 0) {
			...
			if((this.dire == -1 || basis.isPvp()) && basis.canon.deco == DECO_BASE_CURSE) {
	...
	private boolean receive(int dire) {
		return (basis.isPvp() ? this.dire : traitType()) != dire;
	}
