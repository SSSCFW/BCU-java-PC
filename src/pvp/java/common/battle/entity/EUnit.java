...
public class EUnit extends Entity {
	...
	public static class OrbHandler extends BattleObj {
		...
		public static float getOrbMassive(AttackAb atk, ArrayList<Trait> traits, Treasure t) {
		...
		public static float getOrbGood(AttackAb atk, ArrayList<Trait> traits, Treasure t) {
			EUnit eu = getEUnit(atk);
	...
	protected int getDamage(AttackAb atk, int ans) {
        ...
        if (basis.isPvp()) ans = common.battle.PvpDamage.offense(this, atk, ans);
        if (basis.isPvp() && basis.canon.deco == DECO_BASE_WALL && (atk.waveType & (WT_WAVE | WT_MINI)) != 0)
            ans = (int) (ans * basis.b.t().getDecorationMagnification(basis.canon.deco, Data.DECO_WAVE));
        if (basis.isPvp() && basis.canon.deco == DECO_BASE_WATER && (atk.waveType & (WT_VOLC | WT_MIVC)) != 0)
            ans = (int) (ans * basis.b.t().getDecorationMagnification(basis.canon.deco, Data.DECO_SURGE));
	...
	protected float getLim() {
		return Math.max(0, (basis.isPvp() && dire == 1 ? pos : basis.st.len - pos) - ((MaskUnit) data).getLimit());
	}
