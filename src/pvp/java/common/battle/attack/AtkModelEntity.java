		...
		if (abis[ind] == 1) {
			if (getProc(ind).TIME.prob != 0 && getProc(ind).TIME.perform(b.r)) {
				if (getProc(ind).TIME.intensity > 0) {
					b.world().temp_s_stop = Math.max(b.world().temp_s_stop, getProc(ind).TIME.time);
					b.world().temp_inten = getProc(ind).TIME.intensity;
				} else {
					b.world().sn_temp_stop = Math.max(b.world().sn_temp_stop, getProc(ind).TIME.time);
					b.world().temp_n_inten = (float)Math.abs(getProc(ind).TIME.intensity) / b.world().sn_temp_stop;
				}
			}
