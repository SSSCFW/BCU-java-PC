    ...
    public void update() {
        ...
        if (t == 0) {
            ...
            if (rem > 0)
                ...
                activate = false;
            start += move * (sb.isPvp() ? 1 : -atk.model.getDire());
            if (rept == 0) {
                atk.incl.clear();
                rept = rep;
            }
            sb.getAttack(sb.isPvp() && atk.model.getDire() == 1
                ? new AttackWave(atk.attacker, atk, pos, end, start)
                : new AttackWave(atk.attacker, atk, pos, start, end));
            tempAtk = true;
        }
    }
