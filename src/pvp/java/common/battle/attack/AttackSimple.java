...
public class AttackSimple extends AttackAb {
	 ...
	 */
	private final Set<AbEntity> attacked = new java.util.LinkedHashSet<>();
	...
	public void excuse() {
		...
		if (!capt.isEmpty() && proc.WAVE.exists()) {
			int dire = model.getDire();
			int wid = !model.b.isPvp() && dire == 1 ? W_E_WID : W_U_WID;
			float addp = (!model.b.isPvp() && dire == 1 ? W_E_INI : W_U_INI) + wid / 2f;
