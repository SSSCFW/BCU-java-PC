		...
		if (resist < 100) {
			if (proc.id == null || proc.id.cls == Unit.class) {
				...
				if (conf.same_health && ent.health <= 0)
					return;
				int time = proc.time;
				if (b.entityCount(b.ownDirection()) < b.maxNum - u.forms[Math.max(0, proc.form - 1)].du.getWill() || conf.ignore_limit) {
					...
					eu.added(b.ownDirection(), (int) up);
				...
				if (allow >= 0 || conf.ignore_limit) {
					...
					ee.added(b.isPvp() ? -b.ownDirection() : 1, (int) ep);
