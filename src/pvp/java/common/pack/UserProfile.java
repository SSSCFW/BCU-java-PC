...
public class UserProfile {
	...
	public static Collection<UserPack> getUserPacks() {
		String match = common.battle.PvpTiming.currentMatch();
        if (match == null) return profile().packmap.values();
        java.util.List<UserPack> packs = new java.util.ArrayList<>();
        profile().packmap.entrySet().stream().filter(e -> e.getKey().startsWith("pvp_" + match + "_s"))
            .sorted(java.util.Map.Entry.comparingByKey()).forEach(e -> packs.add(e.getValue()));
        return packs;
	}
